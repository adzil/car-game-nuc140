...
void GPAB_IRQHandler(void)
{
	...
	u32GPAStatus = PA->ISRC;
	u32GPBStatus = PB->ISRC;

    /* Avoid to clear EINT0/EINT1 INT flag */
    u32GPBStatus = u32GPBStatus & ~(0x3 << 14);

    /* Clear the interrupt */
    PA->ISRC = u32GPAStatus;
    PB->ISRC = u32GPBStatus;

    /* Call the callback function of PAB interrupt */
    if ( _pfGPABCallback )
...
void GPCDE_IRQHandler(void)
{
    ...
    /* Keep the interrupt source */
	u32GPCStatus = PC->ISRC;
	u32GPDStatus = PD->ISRC;
	u32GPEStatus = PE->ISRC;

    /* Clear the interrupt */
    PC->ISRC = u32GPCStatus;
    PD->ISRC = u32GPDStatus;
    PE->ISRC = u32GPEStatus;
...
int32_t DrvGPIO_SetBit(E_DRVGPIO_PORT port, int32_t i32Bit)
{
	...
	tGPIO = (GPIO_T *)((uint32_t)PA + (port*PORT_OFFSET));  
...
int32_t DrvGPIO_GetBit(E_DRVGPIO_PORT port, int32_t i32Bit)
{
    ...
    u32Reg = (uint32_t)&PA->PIN + (port*PORT_OFFSET);    
...
{    
    ...
    return ((inpw((uint32_t)&PA->DOUT + (port*PORT_OFFSET))>>i32Bit) & 0x1);
}
...
int32_t DrvGPIO_GetPortDoutBits(E_DRVGPIO_PORT port)
{
    ...
    return inpw((uint32_t)&PA->DOUT + (port*PORT_OFFSET));
}
...
int32_t DrvGPIO_SetBitMask(E_DRVGPIO_PORT port, int32_t i32Bit)
{
    ...
    u32Reg = (uint32_t)&PA->DMASK + (port*PORT_OFFSET);    
...
{
    outpw((uint32_t)&PA->DMASK + (port*PORT_OFFSET), DrvGPIO_GetPortMask(port) | i32MaskData);
...
{
	return inpw((uint32_t)&PA->DMASK + (port* PORT_OFFSET)); 
}
...
{
    volatile uint32_t u32Reg = (uint32_t)&PA->OFFD + (port*PORT_OFFSET);
...
int32_t DrvGPIO_EnableDebounce(E_DRVGPIO_PORT port, int32_t i32Bit)
{
    ...
    u32Reg = (uint32_t)&PA->DBEN + (port*PORT_OFFSET);    

    outpw(u32Reg, inpw(u32Reg) | (1<<i32Bit));

	GPIO->DBNCECON.ICLK_ON = 1;
...
int32_t DrvGPIO_SetDebounceTime(uint32_t u32CycleSelection, E_DRVGPIO_DBCLKSRC ClockSource)
{
	...
	GPIO->DBNCECON.DBCLKSEL = u32CycleSelection ; 

	GPIO->DBNCECON.DBCLKSRC = ClockSource ; 
...
int32_t DrvGPIO_EnableInt(E_DRVGPIO_PORT port, int32_t i32Bit, E_DRVGPIO_INT_TYPE TriggerType, E_DRVGPIO_INT_MODE Mode)
{
    ...
    u32Reg = (uint32_t)&PA->IEN + (port*PORT_OFFSET);
    ...
    u32Reg = (uint32_t)&PA->IMD + (port*PORT_OFFSET);
	if(Mode == E_MODE_EDGE)
		outpw(u32Reg, inpw(u32Reg) & ~(1<<i32Bit));
    ...
    {
        NVIC_SetPriority(GPCDEF_IRQn, (1<<__NVIC_PRIO_BITS) - 2);
...
void DrvGPIO_EnableEINT0(E_DRVGPIO_INT_TYPE TriggerType, E_DRVGPIO_INT_MODE Mode, GPIO_EINT0_CALLBACK pfEINT0Callback)
{
    ...
    u32Reg = (uint32_t)&PA->IEN + (E_GPB*PORT_OFFSET);
	if (TriggerType == E_IO_RISING)
    {
    ...
    {  
     ...
     /* Configure to be level trigger or edge trigger */
    u32Reg = (uint32_t)&PA->IMD + (E_GPB*PORT_OFFSET);
	if (Mode == E_MODE_EDGE)
